...
namespace AMS.UI.SoftMask
{
    ...
    public class UISoftMaskEditor : Editor
    {
        ...
        private readonly List<Material> m_InvalidMaterials = new();

        public List<string> m_ExcludeProperties = new List<string> { "m_Script", "m_SoftMaskBlitMaterial" };
                ...
                if (fontMaterialData is { Instances: { } instances })
                    foreach (var material in instances)
                        if (material.Value is var fontMaterial && !MaterialHasSoftMask(material.Value))
                        {
                            ...
                            EditorGUILayout.BeginHorizontal();
                            EditorGUILayout.HelpBox(
                                $"\nTMP material [{fontMaterial.name}]" + m_InvalidFontMaterialMessage,
                                MessageType.Warning);
                            EditorGUILayout.EndHorizontal();
                            break;
                        }
